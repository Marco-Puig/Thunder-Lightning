...
class Pokemon_Dialog < DialogHost
  ...
  TEXT_SPLIT = "("
  TYPE_FORMAT = '%d - %s'
  EXP_TYPES = ["0 - Fast", "1 - Medium", "2 - Slow", "3 - Parabolic", "4 - Erratic", "5 - Fluctuating"]
    ...
    if(v and v.class == Array)
      v.each do |i|
        ...
        begin
          ev = true
          if(id = i[:trade])
            str << "\r\n- #{GameData::Text._get(@lang_id, 0, id.to_i)} par échange"
          else
            if i[:gemme]
              str << "\r\n- Méga-#{GameData::Text._get(@lang_id,0,@pokemon_id)}"
            else
              id = i[:id].to_i
          ...
          msgbox("This Pokémon can not evolve into an undefined Pokémon. (Evolution spéciale : n°#{i[:trade] || i[:id]})", self.class.to_s)
        end
        #> Echange avec
        if(id = i[:trade_with])
          begin
            str << " contre un #{GameData::Text._get(@lang_id,0,id)}"
          rescue TextDataError
            ###
          end
        end
        #> Autres infos
        if(id = i[:min_level])
          str<<" en ayant au minimum le niveau #{id},"
        end
        if(id = i[:max_level])
          str<<" en ayant au maximum le niveau #{id},"
        end
        if(id = i[:item_hold])
          str << " en portant l'objet #{GameData::Text._get(App.lang_id,12,id)}," rescue ""
        end
        if(id = i[:min_loyalty])
          str<<" en ayant un bonheur minimum de #{id},"
        end
        if(id = i[:max_loyalty])
          str<<" en ayant un bonheur maximum de #{id}," rescue ""
        end
        if(id=i[:skill_1])
          str<<" en ayant le skill #{GameData::Text._get(App.lang_id,6,id)},"  rescue ""
          if(id=i[:skill_2])
            str<<" le skill #{GameData::Text._get(App.lang_id,6,id)},"  rescue ""
            if(id==i[:skill_3])
              str<<" le skill #{GameData::Text._get(App.lang_id,6,id)},"  rescue ""
              if(id==i[:skill_4])
                str<<" et le skill #{GameData::Text._get(App.lang_id,6,id)},"  rescue ""
              end
            end
          end
        end
        if(id=i[:weather])
          str<<" si la météo est #{id},"
        end
        if(id=i[:env])
          str<<" si le héros se trouve sur le system tag #{id},"
        end
        if(id=i[:gender])
          str<<" en ayant le sexe #{Gender[id]},"
        end
        if(id=i[:stone])
          str<<" en appliquant une #{GameData::Text._get(App.lang_id,12,id)}," rescue ""
        end
        if(id=i[:day_night])
          str<<" au moment de la journée #{DayTime[id]},"
        end
        if(id=i[:func])
          str<<" en validant la fonction scriptée : #{i[:func]},"
        end
        if(id=i[:maps])
          str<<" sur l'une des maps #{id},"
        end
        str.chomp!(",")
        str << "."
        #<<<
  ...
  def _del
    if(confirm("Any deletion can cause bugs in the game.\nAre you sure you want to delete this Pokémon?", self.class.to_s))
      if @pokemon_id == ($game_data_pokemon.size-1) and @pokemon_id > 1
        ...
        msgbox("Le Pokémon sera vidé.", self.class.to_s)
  ...
  def define_basic_object(pkm)
    ...
    pkm.breed_moves=[]
    pkm.hatch_step=5120
    pkm.rareness=pkm.base_hp=pkm.base_atk=pkm.base_dfe=pkm.base_spd=pkm.base_ats=pkm.base_dfs=pkm.base_exp=pkm.base_loyalty=pkm.female_rate=40
    pkm.height=1.2
    pkm.weight=36.2
    pkm.items=[]
    pkm.master_moves = []
